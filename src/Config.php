...
class Config
{
    ...
    public static function loadFromDir(string $configPath, array $excludeFile = []): array
    {
        ...
        foreach ($iterator as $file) {
            ...
            // 检查是否是一个目录、文件扩展名是否不是 'php' 或者文件名是否在排除列表中
            if (is_dir($filepath) || $file->getExtension() != 'php' || in_array($file->getBaseName('.php'), $excludeFile)) {
                // 如果 $file 是一个目录，或者文件扩展名不是 'php'，或者文件名在排除列表 $excludeFile 中
                // 则跳过当前迭代，不执行后续代码
                continue;
            }
     ...
     * @return array|mixed
     */
    protected static function find(array $keyArray, mixed $stack, mixed $default)
    {
        if (!is_array($stack)) {
            return $default;
        }
